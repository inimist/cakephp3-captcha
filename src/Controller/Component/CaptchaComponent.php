...
class CaptchaComponent extends Component
{
     ...
     * @var array
     */
    private $themes = array('default' => array('bgcolor' => array(200, 200, 200), 'txtcolor' => array(10, 30, 80), 'noisecolor' => array(60, 90, 120)));
    ...
    {
        $this->Controller->helpers['Captcha.Captcha'] = array_merge($this->__getSettings(), array(
            'plugin' => 'Captcha',
            'controller' => 'Captcha',
            'action' => 'create'
        ));
    ...
    private function __mathCaptcha()
    {
        ...
        if ($this->__TTFEnabled()) {
            $angle = 0;
            $textbox = imagettfbbox($fontsize, $angle, $this->settings['font'], $stringOperation) or die('Error in imagettfbbox function');
            $x = ($width - $textbox[4]) / 2;
            $y = ($height - $textbox[5]) / 2;
            $x -= 4;
            $y -= 1;
            imagettftext($image, $fontsize, $angle, $x, $y, $text_color, $this->settings['font'], $stringOperation) or die('Error in imagettftext function');
        } else if (function_exists("imagestring")) {
            //$fontsize = imageloadfont($this->settings['font']);
            $textbox = imagestring($image, 5, 5, 5, $stringOperation, $text_color) or die('Error in imagestring function');
        } else {
    ...
    function __imageCaptcha()
    {
        ...
        $image = @imagecreate($width, $height) or die('Cannot initialize new GD image stream');
        /* set the colours */
        imagecolorallocate($image, $this->themes[$theme]['bgcolor'][0], $this->themes[$theme]['bgcolor'][1], $this->themes[$theme]['bgcolor'][2]);
